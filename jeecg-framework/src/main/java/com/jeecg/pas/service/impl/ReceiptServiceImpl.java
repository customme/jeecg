...
public class ReceiptServiceImpl extends CommonServiceImpl implements ReceiptServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(ReceiptEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(ReceiptEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(ReceiptEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(ReceiptEntity t){
		...
		map.put("discount_amount", t.getDiscountAmount());
		map.put("buy_time", t.getBuyTime());
		map.put("pay_mode", t.getPayMode());
		map.put("currency", t.getCurrency());
		map.put("actual_amount", t.getActualAmount());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,ReceiptEntity t){
 		...
 		sql  = sql.replace("#{discount_amount}",String.valueOf(t.getDiscountAmount()));
 		...
 		sql  = sql.replace("#{actual_amount}", String.valueOf(t.getActualAmount()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("ps_receipt",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
