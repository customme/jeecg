	...
	public StringBuffer end() {
		...
		if(!treegrid && isShowSubGrid){
			...
			if(StringUtils.isNotBlank(extendParams)) {
				int beginIndex = extendParams.indexOf("subTab");
				if(beginIndex > -1) {
					int endIndex = extendParams.indexOf("}", beginIndex);
					String subStr = extendParams.substring(beginIndex + 7, endIndex + 1);
					JSONObject subObj = JSONObject.fromObject(subStr);
					if(subObj.containsKey("width")) {
						subWidth = subObj.getInt("width");
					}
					if(subObj.containsKey("height")) {
						subHeight = subObj.getInt("height");
					}
				}
			}
			
			sb.append("'<div class=\"easyui-tabs\"   style=\"height:" + subHeight + "px;width:" + subWidth + "px;\">'+");
...
//					}
					...
					String align = "left";
					Object extendJson = map.get("extend_json");
					if(extendJson != null && extendJson != "") {
						String[] pair = extendJson.toString().split("=");
						if("align".equals(pair[0])) {
							align = pair[1].replaceAll("\"", "");
						}
					}
					columns += "{title:'"+map.get("content")+"',field:'"+map.get("field_name")+"',align:'" + align + "',width:" + map.get("field_length");
					if("N".equals(map.get("is_show_list"))) {
						columns += ",hidden:true";
					}
					columns += "},";
				}
			...
			for (DataGridColumn col : columnList) {
				if (col.isQuery()&&col.getDefaultVal()!=null&&!col.getDefaultVal().trim().equals("")) {
					...
					if(!"group".equals(col.getQueryMode())){
						...
						String[] arr;
						if(col.getDefaultVal().contains(",") && !col.getDefaultVal().trim().equals(",")){
							arr = col.getDefaultVal().split(",");
						}else{
							arr = new String[]{col.getDefaultVal(),""};
						}
						queryParams += col.getField() + "_begin:'" + arr[0]+"',"+col.getField()+"_end:'"+arr[1]+"',";
					}

				}
			}
			if(StringUtils.isNotBlank(queryParams)){
				int offset = sb.indexOf("queryParams");
				if(offset > -1) {
					sb.insert(offset + "queryParams:{".length(), queryParams);
				}else {
		...
		if("group".equals(getQueryMode())){
			for (DataGridColumn col : columnList) {
				if (col.isQuery()) {
					...
					sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: " + px + "px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
...
//								}
							...
							//	<input type="text" name="order_code"  style="width: 100px"  class="searchbox-inputtext" value="" onClick="inputClick(this,'account','user_msg');" />
								...
								if (null != showMode && "radio".equals(showMode)) {
									...
									sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\" style=\"width: " + getWidth(col.getExtend()) + "px\"> ");
									sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", MutiLangUtil.getLang("common.please.select")));
									for (Map<String, Object> map : list){

										if(col.getDefaultVal()!=null&&col.getDefaultVal().trim().equals(map.get("field").toString())){
						...
						if(this.DATE_FORMATTER.equals(col.getFormatter())){
							sb.append("<input type=\"text\" name=\""+col.getField()+"_begin\" value=\"" + arr[0] + "\" style=\"width: 100px\" "+extendAttribute(col.getExtend())+" class=\"Wdate\" onClick=\"WdatePicker()\"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input type=\"text\" name=\""+col.getField()+"_end\" value=\"" + arr[1] + "\" style=\"width: 100px\" "+extendAttribute(col.getExtend())+" class=\"Wdate\" onClick=\"WdatePicker()\"/>");
						}else if(this.DATETIME_FORMATTER.equals(col.getFormatter())){
							sb.append("<input type=\"text\" name=\""+col.getField()+"_begin\" value=\"" + arr[0] + "\" style=\"width: 140px\" "+extendAttribute(col.getExtend())+" class=\"Wdate\" onClick=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})\"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input type=\"text\" name=\""+col.getField()+"_end\" value=\"" + arr[1] + "\" style=\"width: 140px\" "+extendAttribute(col.getExtend())+" class=\"Wdate\" onClick=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})\"/>");
						}else{
							sb.append("<input type=\"text\" name=\""+col.getField()+"_begin\" value=\"" + arr[0] + "\" style=\"width: 100px\" "+extendAttribute(col.getExtend())+" class=\"inuptxt\"/>");
							sb.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sb.append("<input type=\"text\" name=\""+col.getField()+"_end\" value=\"" + arr[1] + "\" style=\"width: 100px\" "+extendAttribute(col.getExtend())+" class=\"inuptxt\"/>");
						}
	 ...
	 * @return
	 */
	private int getWidth(String extend) {
		int selectWidth = 120;
		if(StringUtils.isNotBlank(extend)) {
			int beginIndex = extend.indexOf("width");
			if(beginIndex > -1) {
				int endIndex = extend.indexOf("px", beginIndex);
				try {
					selectWidth = Integer.valueOf(extend.substring(beginIndex + 6, endIndex));
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
				...
				{
					sb.append(",formatter:function(value,rec,index){");
					sb.append(" if(value == '合计'){return value;}else{");
					...
					sb.append(" if(value.length<=");sb.append(column.getShowLen());sb.append(") {return value.replace(/\\r\\n|\\n/g,\"<br>\").replace(/\\s/g,\"&nbsp;\")}");
					sb.append(" else{ return '<a title= '+value.replace(/\\r\\n|\\n/g,\"&#10;\").replace(/\\s/g,\"&nbsp;\")+'>'+ value.substring(0,");sb.append(column.getShowLen());sb.append(").replace(/\\r\\n|\\n/g,\"<br>\").replace(/\\s/g,\"&nbsp;\")+'...';}}");
				}
