...
//		--author：龙金波 ------start---date：201504022--------for：增加一个特殊sql参数处理---------------------------------- 
	...
	private static void installHqlJoinAlias(CriteriaQuery cq, Object searchObj,
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				if (judgedIsUselessField(name)|| !PropertyUtils.isReadable(searchObj, name) || MethodUtils.getAccessibleMethod(PropertyUtils.getReadMethod(origDescriptors[i])).isAnnotationPresent(Transient.class)) {
					continue;
				}
				// 如果规则包含这个属性
